...
decl_impl_trait! {
    ...
    pub trait CanParse<'data> {
        ...
        // /// Parses an event header.
        // fn event_header(&mut self) -> Res<header::Event> {
        //     pinfo!(self, "parsing event header");
        //     self.raw_event_header()
        //         .chain_err(|| "while parsing event header")
        // }

        // /// Raw event header parser.
        // ///
        // /// Returns internal error as they are. Should only be called by [`event_header`] which
        // /// enriches the errors.
        // ///
        // /// [`event_header`]: #method.event_header
        // #[inline]
        // fn raw_event_header(&mut self) -> Res<header::Event> {
        //     use bitlab::*;

        //     // Parsing in big endian, we're gonna parse the bits and need them in the same order as
        ...
        //     // - the last 7 bits (low endian) in `code` (big endian)
        //     {
        //         macro_rules! err {
        ...
        //     Ok(header::Event { timestamp, code })
        // }
