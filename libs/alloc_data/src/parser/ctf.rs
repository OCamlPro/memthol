...
pub fn date_from_millis(date: ctf::prelude::Clock) -> Date {
...
pub fn parse(bytes: &[u8]) -> Res<(Init, Vec<Diff>)> {
    ...
    // Maps allocation UIDs to indices in the `new` vector of the current diff.
    //
    // Used when parsing collection, to detect whether we can just update the time of death of an
    // allocation in the current diff. Needs to be cleared at the end of each packet.
