...
pub mod nu_footer {
    ...
    pub fn render(model: &Model, footer: &footer::Footer, filters: &filter::Filters) -> Html {
        match footer.active {
            None => html! {
                <footer
                    id = "collapsed_footer"
    ...
    pub mod menu {
        ...
        pub mod subfilters {
            ...
            pub fn render_key(model: &Model, uid: filter::FilterUid, sub: &SubFilter) -> Html {
                ...
                info!("rendering key {}", selected.as_ref().unwrap());
                html! {
                    <Select<SubKey>
                        options = options
                        selected = selected
            ...
            pub mod label {
                    ...
                    Update: Fn(Res<LabelFilter>) -> Msg + 'static + Clone,
                {
                    ...
                    for (idx, spec) in sub.specs().iter().enumerate() {
                        ...
                        table_row.push_value(input::string_input(model, &value, {
                            let msg = msg.clone();
                            let sub = sub.clone();
        ...
        pub mod tabs_center {
            ...
            pub fn render(
                ...
                let callback = |uid: filter::LineUid| {
                    model.link.callback(move |_| {
                        msg::FooterMsg::toggle_tab(footer::FooterTab::filter(uid))
                    })
                };
